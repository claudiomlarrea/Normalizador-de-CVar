...
RE_CVAR_FOOTER = re.compile(
    r"^\s*CVar\s+ES\s+UNA\s+INICIATIVA\s+DEL\s+MINISTERIO\s+DE\s+CIENCIA",
    re.IGNORECASE
)
RE_FECHA_GENERACION = re.compile(
    r"^\s*Fecha\s+de\s+generaci[oó]n\s*:\s*\d{1,2}/\d{1,2}/\d{4}\s*$",
    re.IGNORECASE
)
RE_HTML_ENTITY = re.compile(r"&#\d+;")
RE_NULL_LINE = re.compile(r"^\s*null\.?\s*$", re.IGNORECASE)

RE_NAME_HEADER = re.compile(
    r"^\s*[A-ZÁÉÍÓÚÑÜ][A-ZÁÉÍÓÚÑÜ\s\-']+,\s*[A-ZÁÉÍÓÚÑÜ][A-ZÁÉÍÓÚÑÜ\s\-']+\s*$"
)
...
def _should_force_new_paragraph(line: str) -> bool:
    ...
    if line.lower().startswith("rol:"):
        return True
    if re.match(
        r"^(Organizada por|Ejecutado en|Financiado por|Direcci[oó]n|Instituci[oó]n de ejecuci[oó]n|Tareas realizadas)\s*:",
        line,
        re.IGNORECASE
    ):
        return True
